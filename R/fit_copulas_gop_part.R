...
loglik_df <- as.data.frame(matrix(ncol = 15*5, nrow = 9))
loglik_part_df = data.frame(P = rep(NA, 15*5),strategy = rep(NA, 15*5), ll = rep(NA, 15*5), copula = rep(NA, 15*5), params = rep(NA, 15*5))
...
write.csv(loglik_df, "exchange/loglik_df_gop_no_iteration.csv")
write.csv(loglik_part_df, "exchange/loglik_part_gop_no_iteration.csv")
