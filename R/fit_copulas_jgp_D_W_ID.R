...
# W
grouped <- df %>% group_by(A, W)
grouped_data <- grouped %>% group_split()

loglik_df_condition <- data.frame(D = rep(NA, 12*11), W = rep(NA, 12*11), ll = rep(NA, 12*11), copula = rep(NA, 12*11), params1 = rep(NA, 12*11), params2 = rep(NA, 12*11))
...
for (group in grouped_data){
  ...
  col_index = 1
  loglik_df_condition[df_index,] = c( a, w, 0, "Independent", NA, NA)
  df_index = df_index +1
  ...
  for (fit in fits){
    ...
    loglik_df_condition[df_index + col_index-1,] = c(a, w, fit_ll_value, copname, copparams)
...
library(formattable)
write.csv(loglik_df_condition, "exchange/loglik_df_jgp_D_W.csv")
