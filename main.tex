...
\begin{abstract}
  ...
  In this work, we instead adopt a \textit{probabilistic} perspective to model the \textit{joint distribution} of MTs and \ide, and how this relationship depends on \textit{user strategy}.
  We re-analyze data from four empirical studies that used three different experimental protocols that manipulated task constraints and user strategy across different input modalities, examining how speed and accuracy are traded off locally (within conditions) and globally (across conditions). We find strong global dependencies but much weaker local dependencies, suggesting movements within a condition are largely functionally equivalent.
...
It has been validated across devices, modalities, and populations, and has been a cornerstone of input evaluation and interface design \cite{card1978,soukoreff2004}.
...
Fitts' law also leaves no room for user strategy, where participants may favor speed or accuracy depending on instructions, cost of errors~\cite{bertucco2015}, or personal preference \cite{zhai2004nominal,guiard2015}, which conflates intentional and unintentional variability. As we will show, variability in MT and user strategy strongly influence observed responses: ignoring them oversimplifies pointing behavior and limits the predictive power of models.
...
Rather than assuming that going faster always means being less accurate, or that smaller targets always lead to lower MTs, we examine how user strategies and task properties influence the dependence between the observed MTs and accuracy.

In the first part of this work, we focus on describing the speed-accuracy tradeoff by characterizing how variability arises \textit{within} and \textit{across} D and W, and strategies.
...
To model this dependence, we rely on the exponentially modified Gaussian (EMG) model and \textit{copulas}. EMG is a parametric model that was previously validated to model MT distributions~\cite{gori2019,gori2018these,li2024,zhao2022}, but without accounting for strategy. While the EMG model captures important aspects of the SAT, it also imposes the form of dependence a priori.
Copulas, which describe the joint distribution of MT and accuracy without assuming a fixed pattern of dependence, offer more flexibility.
Together, these approaches let us compare different shapes and strengths of the SAT across experimental conditions.
In a second part, starting from \autoref{sec:gen}, we shift to a predictive perspective: using what we have learned about these dependencies, we build and compare three generative models of pointing.
These models are evaluated on how well they can reproduce the key statistical features of real datasets.
This dual approach reflects the distinction between explanatory modeling and prediction \cite{shmueli2010}.
...
(2) a \emph{pure strategy} protocol, where accuracy is self-determined by participants in line with experimenter instructions; and
...
\begin{itemize}
  \item A theoretical analysis that shows the limitations of using Pearson correlation to evaluate the fitness of models that predict the joint distribution between MT and accuracy.
  ...
  \item Empirical findings across three protocols, that show that the SAT is often weak locally but emerges globally.
...
By moving beyond deterministic, strategy-agnostic models that describe average movement times, this work provides a richer account of the SAT in pointing that better reflects the variability and strategies observed in real interaction.
...
Many studies have extended Fitts' law, and namely its applicability to novel paradigms such as 2D and 3D pointing, by proposing a novel index of difficulty. One of the most used extension is the effective index of difficulty \ide presented next.
Other examples include \citeauthor{accot2003}~\cite{accot2003}, who review several indices for bivariate pointing, with additional insights provided by~\cite{grossman2005b}, and \citeauthor{murata2001}~\cite{murata2001} who extend the framework to trivariate pointing.
...
\end{align}
ID$_e$, as a function of $\sigma$, can be interpreted as a measure of accuracy that may only be computed a posteriori from observed data.
...
\begin{align}
  I_U = \log \frac{W_e}{W}.
\end{align}
$I_u$ measures the effect of strategy, because, as shown in \cite[Appendix 3]{gori2018tochi}, $W_e = \frac{2.066}{\sqrt{2}\text{erf}^{-1}(1-\varepsilon)} W$ when the spatial distribution of endpoints is Gaussian, and denoting $\varepsilon$ as the target miss rate (also see~\cite{soukoreff2004, gori2020}). Thus, $I_u$ is a non-linear transformation of the miss rate $\varepsilon$.
The key finding from \citeauthor{zhai2004nominal}~\cite{zhai2004nominal} for our purposes is that the post-hoc correction \ide depends on user strategies. Moreover, they showed that D and W have effects on movement time beyond through their ratio D/W. These results imply that \autoref{eq:ide} is a simplification, and could be extended by treating \ide as a quantity shaped jointly by ID, D, W, and participant strategy. This observation motivates our modeling of its distribution, see \autoref{sec:outline}.
...
Rather than extending WHo model to account for distributions, which would be difficult because of how the model is parametrized, we adopt another modeling approach that retains an explicit strategy parameter.
...
The expression for Fitts' law \autoref{eq:ide} masks an underlying statistical model that assumes additive, centered noise~\cite{soukoreff2004}\footnote{Because most researchers applying Fitts' law rely on ordinary least squares (OLS), they also implicitly assume the noise is uncorrelated and homoscedastic (has equal variance), as OLS is only optimal under these conditions.}:
\begin{align} \mt = a + b\ide + \text{noise (additive)}, \quad \mathbb{E}[\text{noise}] = 0 \text{ (centered)}. \label{eq:mt}
\end{align}
Pearson's $r$\footnote{Some researchers consider the coefficient of determination $R^2$, which reflects the proportion of variance in the dependent variable that is explained by the model while simultaneously assessing the model’s linearity~\cite{lee1988}. For linear regression, it is well known that $R^2 = r^2$} is commonly used as a goodness-of-fit measure. % because for linear models with OLS $r^2 = R^2$, where $R^2$ is the so-called coefficient of determination which reflects the proportion of variance in the dependent variable that is explained by the model while simultaneously assessing the model’s linearity~\cite{lee1988}.
...
In fact, to mitigate variability in MT and increase the goodness-of-fit measure, researchers typically compute $r$ on the \textit{sample average} of MT, denoted in the remainder of the work as \mmt\footnote{See~\cite{gori2017two} for further discussion on considering the sample average.}. Consequently, the model actually considered by many researchers is:
\begin{align}
  \mmt = a + b\ide + \text{noise (additive)}, \quad \mathbb{E}[\text{noise}] = 0 \text{ (centered)}, \label{eq:mmt}
\end{align}
with the goodness-of-fit measure being $r(\mmt, \ide)$. Thus Fitts' law is a model for average pointing time \mmt.

\subsubsection{Asymmetric MT distributions and the Exponentially Modified Gaussian (EMG) noise model \label{subs:rw::emg}}
Other works consider the entire MT distribution: the conditional distribution of MT for a given ID or \ide level is found to be asymmetric and positive (right) skewed~\cite{gori2018these,gori2019,zhao2022, jude2016,nieuwenhuizen2016, li2024,chapuis2007}.
...
\end{align}
where erfc is the complimentary error function, $\mathbf{x} = \left[1, \ide \right]^T$, $\lambda = \left[ \lambda_0, \lambda_1 \right]$ and $s$ are variance terms, and $\beta = \left[ a, b\right]$.
The EMG model continuously interpolates between a Gaussian distribution ($s >> \mathbf{\lambda} \cdot \mathbf{x} $) and an exponential distribution ($s <<  \mathbf{\lambda} \cdot \mathbf{x}$).
Although this density is analytically quite complex, it has a simple conditional mean, where \mmt increases linearly with \ide
\begin{align}
...
\end{align}
Thus, for the EMG distribution, the shape is determined by the relative contributions of $s$ and $ \mathbf{\lambda}$, the location is determined by $\mathbf{\beta}$ and $\mathbf{\lambda}$, and the scale by $s$ and $ \mathbf{\lambda}$. Throughout this work, we fitted the EMG distributions when needed with maximum likelihood estimation.
...
Optimal feedback control (OFC) as described originally by Todorov and Jordan~\cite{todorov2002} in the context of human movements have seen large popularity in motor control, and have been applied in HCI contexts recently~\cite{klar2023,fischer2022}. These predict complete movement trajectories as well as their spatial variability.
Unfortunately, these models are not well suited to predict MT and MT variability: since OFC doesn't have an intrinsic mechanism to ``stop controlling'', it requires introducing a stopping condition (see \eg \cite[Section 3.4]{qian2013}). \footnote{There are two possibilities: either a fixed horizon is used, which requires specifying in advance the movement duration, or an infinite or receding horizon is used. In that case, movements are perpetual, except if a stopping condition (usually arbitrary) is formulated.}
...
Reinforcement learning approaches to modeling pointing~\cite{ikkala2022, charaja2024, fischer2021,do2021,moon2024}, which can be viewed as methods to solve optimal control problems when the dynamics of the controled system are unknown, can produce data compatible with Fitts' law~\cite{ikkala2022, charaja2024, fischer2021}, but to our knowledge, they have not shown to be able to generate realistic MT distributions,\footnote{Of the three mentioned references, \citeauthor{fischer2021}~\cite{fischer2021} come closest; they display a boxplot for MT, which shows that the medians are a linear function of ID, and shows MT distribution that are right skewed, both in line with known patterns of MT distributions. However, the variance of MT does not seem to increase with ID, in fact the smallest ID levels seems to have the highest levels of variance, which is not in line with empirical observations.} and are less interpretable than parametric models. They also require extensive training time (\eg 6 days for~\cite{do2021}, 40 hours for~\cite{moon2024}) and it is unclear how well they transfer to new tasks (although see~\cite{moon2024, bhattarai2026} for examples of training times within minutes).
Thus, while simulation models can be used to generate movement time trial data, to our knowledge, it is unclear whether they can generate realistic MT distributions. Moreover, while it should be possible, it remains a question how these models could be adapted to account for user strategy. Finally, their lengthy training times and the efforts required for their implementation undermine their utility as simple, parametric models, which are the focus of this work.

\section{Preliminaries}

\subsection{Theoretical analysis of the $r^2(\mmt, \ide)$ correlation \label{subs:theory_rsq}}

The goodness of fit of Fitts' law is usually justified via the square of the Pearson correlation $r^2(\mmt, \ide)$, which is close to 1 in many experiments where accuracy is manipulated by W~\cite{soukoreff2004}.
However, we show that many different MT distributions can achieve $r^2(\mmt, \ide)$ close to 1. Thus, using alignment with Fitts' law is a weak criterion to assess the fit of a model for distribution of movement times.
...
The proof is given in Appendix \ref{app:proofs}, and relies on the fact that Pearson correlation is a function of the covariance, which measures linear deviations from the mean. Several distributions that have been previously used for modeling pointing times satisfy this condition
\begin{itemize}
  \item The EMG model with quadratic variance~\cite{gori2018these,gori2019, li2024,zhao2022} has linear conditional mean (\autoref{eq:emg_mean}).
...
Two important takeaways follow: First, models of movement time must have a conditional mean linear in \ide to satisfy Fitts' law. For example, if one has reasons to consider a gamma distribution for MT with both k and $\theta$ that increase with \ide, then a possibility is to consider a square root model such as $\theta = \omega_1 \sqrt{\ide}$, $k = \omega_2 \sqrt{\ide}$, so that the product $k\theta$ increases linearly with \ide.
Second, it motivates the need to turn to measures of dependences that are more sophisticated than Pearson correlation, which is where the copulas will come into play.
...
\autoref{fig:copulas_explained} illustrates how the Gaussian copula shapes a joint distribution: In (d), the two marginals (a) and (b) are independently combined. In (e), the two marginals are coupled with a Gaussian copula (c). For this illustration, we have used realistic marginal distributions for MT (a) and ID (b). Note that in both jointplots (d) and (e), the estimated marginals are identical, but notice how the actual datasets differ. In particular, we see that the dataset obtained with the Gaussian copula has variance that increases with ID levels, which is characteristic of pointing data~\cite{gori2018these}.
...
The way we identify copulas in this work is further described in Appendix \ref{app:copula:fit}. All copulas used in this work are displayed for two sets of marginal distributions in \autoref{fig:copula_visualization}: standard normal marginals, which is a classical way of representing copulas, and marginals estimated from a pointing dataset. The latter in particular allows one to appreciate the different shapes of the datasets produced by the selected copulas.
...
In this first part of the work, we model the joint distribution of movement time (MT) and effective index of difficulty \ide at two levels of analysis, by re-analyzing data from three previously conducted experiments.
...
As we will show in the remainder, these two perspectives lead to different conclusions: within individual conditions, MT and \ide often appear weakly dependent, but when conditions are aggregated, a global trade-off consistent with the SAT emerges.
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}[scale=1.2]
    ...
    \draw[thick, black, ->] (-5.75, -2.5) -- (5.5, -2.5) node[below, pos=.9]{ID$_e$};

  \end{tikzpicture}

  \caption{The difference between the local and global dependency structure. The blue ellipses which represent 95\% confidence ellipses for data associated with one block, being oriented vertically, show no correlation between MT and \ide. The grey ellipsis, which represents the 95\% confidence ellipsis for the aggregated data, being oriented at an angle, shows that MT is correlated with \ide. This shows that data that is locally independent can still lead to dependencies when pooled across conditions. In this example, what causes the dependency is mostly the position of the means of the blue ellipses.}
...
\begin{enumerate}
  \item \textbf{Conditional EMG approach.}
    ...
    \begin{align}
      p(\mt, \ide | \D, \W, s)
      &= p(\mt | \ide, \D, \W, s) \cdot{} p(\ide | \D, \W, s).\label{eq:chain_rule}
    \end{align}
    We then make the simplifying assumption that MT depends on task parameters only through \ide:
    \begin{align}
      p(\mt | \ide, \D, \W, s) = p(\mt | \ide).
    \end{align}
    This reduction avoids considering too many variables and we justify it by the fact that \ide already summarizes task geometry and strategy \ie the remaining effect of s, \D and \W on MT beyond those due to \ide are likely second order.
    ...
    We also model \(p(\ide | D, W, s)\) enabling the model to be used not just descriptively, but \textit{predictively}\footnote{Indeed, one needs a generative model for \ide in this case, to ``supply'' \ide values to the conditional MT distribution.}.
    Because the EMG has already been validated in prior work, our goal here is not to re-compare candidate models for $p(\mt|\ide)$. Instead, we focus on showing how the distinctive features that make the EMG model successful (asymmetry and quadratic variance) contribute to better fits, while placing greater emphasis on modeling $p(\ide| \D,\W,s)$, the other, lesser studied part of the equation. The EMG model will be exploited later in the second, predictive part of the work.
  ...
  \item \textbf{Copula-based approach.}
    ...
    This approach removes the restriction imposed by the EMG variance model and provides a more flexible description of MT and \ide; thus our first approach to measure dependence is based mostly on copula fitting.
\end{enumerate}
...
In the first study (\autoref{sec:jgp}), we re-analyze data of a standard Fitts protocol (Appendix \ref{app:protocols::fitts_protocol}), where accuracy is controlled by target width \W.
...
We considered several candidate copula models to describe the global and local dependencies; we selected the rotated Gumbel copula as the best fit for the global distribution, capturing strong correlation in the lower tail.
For the local distributions, a low-correlation Gaussian copula provided the best fit.
We also propose a linear model relating \ide to ID to form the conditional distribution $p(\ide | \text{ID}, \D, \W)$. This study shows that the speed-accuracy tradeoff emerges globally, not locally.

\paragraph*{Study 2: Pure strategy protocol}
In the second study (\autoref{sec:gop}), we analyze data from a pure strategy protocol (Appendix \ref{app:protocols::guiard_protocol}), where accuracy is determined only by the participant's strategy $s$.
...
Finally, we find that the Gaussian and t-copulas provide the best fit for $p(\mt, \ide | s)$ and $p(\mt, \ide)$, although with much stronger correlation for the global dependence $p(\mt, \ide)$.

\paragraph*{Study 3: Fitts-with-strategy protocol}
In the third study (\autoref{sec:yamanaka}), we combine both approaches to manipulate accuracy, by analyzing data from a Fitts-with-strategy protocol (Appendix \ref{app:protocols::full_protocol}), where accuracy is influenced by both target width \W and strategic instructions $s$.
...
As in the previous study, Gaussian and t-copulas yield the best joint models, again with stronger correlation for the global level.
...
We fitted the EMG model for each participant and for each replication. In line with existing works on this dataset~\cite{gori2018these,zhao2022}, the EMG fit was largely superior to the Gaussian fit obtained via linear regression. We namely found AIC\footnote{Akaike Information Criterion, a standard metric for model comparison~\cite{anderson2004}, see also Appendix \ref{app:copula:compare}.} differences routinely above 300; typically a difference of 10 is already considered strongly discriminative~\cite{anderson2004}. A pair-plot of the fitted parameters can be found in the supplementary materials.
...
We compared candidate copula models using the model evidence ratio $\mathcal{R}$ as explained in Appendix \ref{app:copula:compare}.
When comparing two models, an evidence ratio of $x$ means the better-supported model has $x$:1 odds of being closer to the true data-generating process than the other. This allows us to compare copulas on a common scale.
Combined\footnote{Copulas were fit to each participant and condition, and then combined to compute a model evidence ratio as explained in Appendix \ref{app:copula:compare}. Throughout this work, we refer to this method as \textit{combined}.} over participants and across D$\times$W pairs, the independent copula was most often selected as the best-fitting model (see %\autoref{fig:copula_fit_jgp_dwp},
\autoref{fig:copula_fit_jgp_dw}).
Other copulas have close model evidence ratios; this is because many copulas tend to the independent copula as their parameters reach a certain limit \eg the Gaussian copula tends to the independent copula when the correlation parameter $\rho$\footnote{We use $\rho$ to distinguish the linear correlation parameter of the Gaussian copula, from Pearson correlation $r$. Both are in general not equivalent, but are often qualitatively very similar; they are only equivalent if the marginals of the copula are Gaussian.} tends to 0.
This indicates that within a given condition, longer movement times were not reliably associated with higher effective difficulty. In other words, the local dependency between MT and \ide was weak or absent.

An exception occurred for the smallest target condition ($\W=64$), where both Spearman's $\rho$ and Kendall's $\tau$ correlations were slightly negative. This suggests that in this extreme case, faster movements could even be associated with greater accuracy, the opposite of the expected SAT. While weak, this paradoxical result suggests that the large trial-to-trial variability can obscure or even invert the trade-off locally.
...
To test whether the individual effects of W and D influence \ide beyond their joint contribution through the ratio D/W (ID), we fitted four linear mixed-effects models (LMM) with random intercepts for participants (see Appendix \ref{app:lmm} for an explanation of LMM and how they were fitted and compared): a full model with ID, W, D, and all interactions, two simpler models with only ID+W or ID+D, and a baseline with ID alone.
...
The two simple model fits are shown in \autoref{fig:ide_id_jpg}, which illustrates that D significantly shifts the \ide - ID relationship upwards.
This result diverges from \citeauthor{zhai2004nominal}~\cite{zhai2004nominal}, who reported W effects\footnote{\autoref{fig:ide_id_jpg} suggests an effect of W on the slope of the \ide-ID relation, but this effect did not turn out significant}. Two factors may explain the discrepancy. First, our W values (64--128 px) were larger than theirs (12--72 px), and W-ID interactions likely manifest more strongly at smaller target sizes.\footnote{However, comparisons must be taken cautiously, since pixels alone do not tell the full story, we also have to consider display density and input device characteristics \cite{casiez2011}, for which we do not have the complete information.} Second, fully disentangling W or D from ID requires a shape-scale design \cite{gori2018chi, guiard2009}, where D or W is crossed with ID, which is not the case of this dataset.
Overall, the additional effects of D and especially W beyond ID, while they can be of theoretical interest under specific experimental designs, are likely negligible in most practical contexts. For example, D changes \ide by about 0.3 bits for each 1000 pixels, but this is counteracted by the interaction term as \ide increases: at ID = 3, D changes only 0.296 - 0.12 = 0.176 bit per 1000 pixels, which represents a minor shift, especially when compared to the MT variability.
...
Participants had to aim at a line, with five different strategies, from speed emphasis allowing a large spread of endpoints, to precision emphasis where the goal is to target the 1-pixel-wide line.
...
\subsection{A model between \mmt and \ide parametrized by strategy\label{subs:mmt_ide_strategy}}
...
While aggregating data per strategy yields a very high correlation between \mmt and \ide ($R^2=0.97$), block-level averages show more variability ($R^2=0.58$).
To capture how strategy affects behavior without overly masking this variability, we first model \mmt conditional on strategy. This provides a simple model of strategic effects, useful for simulation and prediction, and helps to build a basis for the subsequent trial-level analysis.
...
We combined the five strategy-specific Gaussian fits into a single model by assigning each strategy a numerical score on [-1,1] where -1 stands for complete speed emphasis, 0 for balanced, and +1 for complete precision emphasis. This simple symmetric coding allows us to examine how the Gaussian parameters vary with strategy, without relying on post-hoc measures such as \ide.
At first glance, treating an ordinal measure such as strategy as numerical may seem inappropriate; however, as we will demonstrate, this transformation leads to a simple and interpretable linear model parameterized by the numerical strategy.
The model parameters are represented as function of coded strategy in \autoref{fig:meancov_strat}. We further compared linear models of each Gaussian parameter against constant (no-effect) models using AIC (\autoref{tab:aic_gop}); the best models are displayed in \autoref{fig:meancov_strat}. It results in three distinct cases:
\begin{enumerate}
  ...
  \item The correlation $r$ is best described as constant across strategies, except for the speed emphasis condition where it collapsed toward zero. In other words, under speed emphasis, accuracy decouples from mean movement time. Although isolating this condition may appear arbitrary, it is well-motivated from a motor control standpoint: speed-focused movements are primarily ballistic~\cite{elliott2017}. The smaller variance in $\sigma_i$ further supports this separation, suggesting greater uniformity in this condition and reinforcing the idea that precision demands introduce additional complexity in movement production.
\end{enumerate}
...
Thus, the Gaussian model and the mapping of strategy to [-1,1] are compatible both with local Fitts' law (within a given strategy, via \autoref{eq:mmt_ide_gauss}) and with global Fitts' law (across strategies, via the linear trend in means). The model therefore provides a bridge between individual strategies and Fitts' law.
...
The results, displayed in \autoref{fig:r_cop_gop_s}, show that the Gaussian copula provided the best local fit, except for the balanced strategy, where the Galambos copula was slightly favored. Although the Galambos family is typically associated with strong upper-tail dependence, the estimated parameter values here ($\theta \approx 0.5$) make it similar to the independent copula, as can be seen \autoref{fig:copula_visualization}. A violin plot of the estimated Galambos copula parameters is provided in the Supplementary Materials, showing low $\theta$ values for most fits.
...
As in the first study, we found strong \textit{global} dependence between \ide and MT when data is pooled across conditions (here, strategy), while within conditions, \textit{local} dependence remains weaker.

We first confirmed that the data from the pure strategy protocol are compatible with Fitts' law. When aggregated by strategy, \ide and \mmt were almost perfectly correlated ($\rho = 0.97$). To capture the effect of strategy, we introduced a bivariate Gaussian model parameterized by strategy, a model that is both simple and interpretable.  The ordinal-to-numerical coding was successful in producing a simple but useful representation, since ``uniform'' differences in strategies lead to a linear repartition of mean \mmt and \ide values, therefore reproducing Fitts' law globally. Since $\ide = \log(1 + \tfrac{D}{4.133\sigma}) \simeq \alpha \log \sigma$ for small $\sigma$, the coding is consistent with a log-linear increase between instructions and $\sigma$, which is in line with a motor-like Weber's law~\cite{kandel}, where accuracy, rather than percepts, are interpreted on a logarithmic scale.
We found mostly weak but positive within-strategy correlations between \ide and \mmt, with the highest correlation in the balanced condition $(\rho \simeq 0.3$), except in the speed-emphasis condition. The latter exception aligns with the idea of ballistic, feedforward movements~\cite{gan1988,crossman1957}, where accuracy no longer relates to movement time.
...
For comparison with the two previous studies, we only used regular (mouse-based) pointing data, and aggregated data across distractor density levels, since the sensitivity of MT in regular pointing to distractor density is low~\cite[Table 2]{blanch2011}.
Twelve participants completed 23 trials per condition, which makes $12 \text{ participants} \times 3 \text{ W} \times 2 \text{ D} \times 3 \text{ strategy} \times 69 \text{ movements} = 216 \text{ sets} \times 69 \text{ movements} = 14904$ trials. The first 3 trial of each set were discarded (648) with some outliers (244), resulting in 14012 data points.
...
We found that one unit of strategy augments the slope by approximately 73 ms/bit. Since one unit of strategy separates the \textit{Speed} and \textit{Accuracy} conditions, this amounts to a change of about 35\,ms/bit between each consecutive condition, consistent with the study from \citeauthor{zhai2004nominal}~\cite{zhai2004nominal} who found differences of about 30ms/bit in a similar study.

\begin{figure}[htbp]
  \centering
  \begin{subfigure}[t]{0.43\textwidth}
    \includegraphics[width=\linewidth, trim = {11cm 0 0 0}, clip]{img/fitts_yamanaka.pdf}
    \caption{Data of the YKORM dataset and adjusted Fitts' law obtained by computing linear regression on MT with \ide and strategy as predictor. The fitted model without noise is MT = -0.24 + 0.266$\cdot{}\ide$ -0.244$\cdot{}s$ + 0.073$\cdot{}s\cdot{}\ide$. Strategies are mapped to $s$ as in \autoref{subs:mmt_ide_strategy} \ie $s=0$ corresponds to the \textit{balanced} strategy, and $s=0.5$ corresponds to the \textit{accuracy} strategy.}
...
\begin{align}
  \text{X} & \sim \text{ID} + \text{strategy (ID+S model)}  \\
...
The model comparisons results, based on AIC, are given in \autoref{tab:aic_yamanaka}:
\begin{itemize}
...
Interestingly, because the intercept is positive and $\beta < 1$, the \ide line will cross the $\ide = \text{ID}$ line. Thus, combined with the fact that high IDs are overwhelmingly obtained with small values of W~\cite{gori2018chi,guiard2009}, this model explains both the over- and under-utilization of targets noted by \citeauthor{zhai2004nominal}, while also accounting for strategic modulation, without having to explicitly include a main effect of W.
...
Pooling across strategies, we obtained 72 datasets. The results (\autoref{fig:R_yamanaka_DW}) show that Gaussian copulas again provide the best overall fit, with a an exception for the smallest width target (W = 8), where rotated Galambos or HR copulas could score higher. The average Gaussian copula correlation was $\rho = 0.41$, indicating a moderate \textit{local} dependency. Unlike in the dataset of the first study, this larger $\rho$ likely reflects the fact data was pooled over strategies. No significant effects of D or W on $\rho$ were found.\footnote{A barplot of the $\rho$ values and the fit details can be found in the supplementary materials.}
...
Across the three studies, copulas consistently revealed a weak local dependence between \ide and MT. This suggests that faster movements can also be more precise, and that this may actually be relatively common. The pure strategy protocol introduced a somewhat stronger local dependence; this suggests that when accuracy is enforced by W, movements are largely ``equivalent'', whereas when accuracy is enforced by strategy, the participants seem to exerce more control, we believe \eg by re-adjusting the strategy based on the previous trial.

In contrast, the global dependence across conditions is strong and robust, in line with the classical speed-accuracy tradeoff. The first study favored the rotated Gumbel copula (mean $\theta \simeq 2$), while the second and third studies favored Gaussian/t-copulas (average $\rho \approx 0.8$). Importantly, these copulas yield visually similar joint distributions (see \autoref{fig:copula_visualization}, where the top right panel and right panel of the third row are hardly dinstinguishable), so the discrepancy does not undermine the main finding: \ide and MT are tightly coupled at the global level, with a MT variance that increases with \ide levels. In the Fitts-with-strategy protocol, the correlations between \ide and MT increases compared to the pure strategy or the classical Fitts protocols whenever data was pooled across (D,W) pairs within a strategy, or pooled across strategies within a (D,W) pair, which is also consistent.

Considering the resulting distributions are very similar visually, could the difference between Gumbel and Gaussian fits be an artifact of model identifiability~\cite{gori2024, wilson2019}? In other words, perhaps the two copulas are hard to distinguish, and small variations may push more towards one or the other copula. To test this, we simulated datasets with realistic marginals and sample sizes comparable to our studies (Appendix \ref{app:identifiability}). The analysis shows that the rotated Gumbel should be identifiable given our data, ruling out a spurious explanation. We therefore attribute the difference to the experimental protocols and input modalities: gesture input in study 1, physical arm movements in study 2, and mouse pointing in study 3. We return to this interpretation in \autoref{subs:synthesis:pdd}.
...
Across studies 1 and 3, we found no consistent evidence that D or W influence \ide beyond their ratio in ID. In standard full-factorial designs, D and W are strongly correlated with ID~\cite{gori2018chi}, which makes disentangling their effects statistically fragile. A shape $\times$ scale design would be needed to test these effects more decisively~\cite{guiard2009}, which we investigate in \autoref{subs:synthesis:pdd}.

Importantly, a model with main effects for W on top of the ratio D/W is not needed, since the linear model proposed in \autoref{eq:model_ide_simple} already captures the main phenomenon of interest described by \citeauthor{zhai2004nominal} of target over- and under-utilization. We therefore advocate this model as a parsimonious and practically useful account. Expressed probabilistically with a standard Gaussian noise formulation, it reads:
\begin{align}
  \ide  \sim \mathcal{N}\left( \alpha + \beta\,\text{ID} + \gamma\,\text{strategy}, \sigma^2 \right).\label{eq:probabilistic_ide}
\end{align}
We assessed the variability of \ide, and found $\sigma \approx 0.3$ for the YKORM dataset and $\sigma \approx 0.15$ for the JGP dataset. We return to the interpretation of these values in \autoref{subs:synthesis:pdd}.
...
To further validate and generalize our findings, we applied our analysis to the Pointing Dynamics Dataset (PDD) \cite{mueller2017}. This dataset is particularly informative because it uses a shape $\times$ scale design (\D = 212, 353 mm; ID = 2, 4, 6, 8 ), which allows to decorrelate D from ID, includes a condition with a very small target size (less than 1\,mm), which we hypothesized was missing from our datasets to compare to \citeauthor{zhai2004nominal}'s results, employs a standard mouse with constant CD gain, and includes a large number of trials per condition (12 participants $\times$ 8 blocks $\times$ 102 trials).
...
We fitted similar mixed-effect linear models as in the first study, with parameter estimates summarized in \autoref{tab:fit_pdd_ide_id}. Compared with the previous studies, the large sample size means the log-likelihood differences are more pronounced, and the effects are all found significant. However, the effect sizes for D and W or their interaction with ID are all small. For example, the effect of W, which is the largest, induces a change of about .4 bits every 100 pixels and the interaction effect of D or W with ID is systematically in the other direction as the main effect, which further reduces the impact of the variable. Thus, in general, these effects can be neglected.
Importantly, the models reproduces the classic target under and over-utilization phenomenon: positive intercepts and slopes slightly below 1 indicate under-utilization for small IDs and over-utilization for large IDs. This confirms that the simple linear model in \autoref{eq:model_ide_simple} can also in this case explain \citeauthor{zhai2004nominal}'s observations.
...
\ide and MT (\autoref{fig:mer_pdd}).
...
As with any generative approach, there is a trade-off between simplicity and realism. Simpler models tend to be more robust and easier to generalize, but may miss ``second-order'' effects and thus be slightly less predictive on the dataset on which it has been trained; here, we prefer simplicity.

Throughout this section, $\theta$ denotes a vector of model parameters, including strategy $s$. $p_{\theta}(\ide, \mt)$ refers to the joint distribution between \ide and MT , and $p_{\theta}(\mt|\ide)$ refers to the conditional distribution of MT given \ide, both parametrized by $\theta$. We present three methods to generate realistic synthetic pointing data, while highlighting their trade-offs in flexibility and interpretability.
We use parameter values estimated from the three studies as a concrete example. Researchers interested in applying these models could likely use the same parameter values, considering the experiments that we considered are rather standard. However, if their context differs, they can easily re-estimate these parameters on their own dataset.

\subsection{Model 1: Joint distributions using Copulas.\label{sub:model1}}

\paragraph{The model.} The first method defines the global dependency $p_{\theta}(\ide, \mt)$ in terms of copulas. As explained in \autoref{sec:copula}, the copula $C_{\theta}$ links the marginals $F_{\theta}(\ide)$ and $G_{\theta}(\mt)$ into the joint $p_{\theta}(\ide, \mt)$
\begin{align}
  p_{\theta}(\ide, \mt) = C_{\theta}(F_{\theta}(\ide), G_{\theta}(\mt)).
\end{align}

Pairs of (MT,\ide) values can then be obtained by sampling directly from the joint distribution. To replicate blocked data, in which multiple MT values correspond to a single \ide value, we first sample \ide from the $p(\ide|s)$ model. Given these values, MT is then sampled from the corresponding conditional copula, which can be computed directly from the copula specification~\cite{nelsen2006}.

\paragraph{Accounting for different strategies.} To have the resulting joint distribution depend on s even though the copula does not, we sample \ide values from the $p(\ide | s)$ model and sample from the conditional copula.
For the $\ide|s$ model, we consider the marginal distribution of the bivariate gaussian model \autoref{eq:gaussian_ide_mt}, which reads, with the parameters found in \autoref{sec:gop}:
\begin{align}
...
\begin{enumerate}
  ...
  \item The parameters of the marginal distributions for MT and \ide, which can be estimated using any popular statistical package. For MT, several asymmetric distributions may be valid; we prefer the EMG distribution as explained \autoref{subs:rw::emg}. For the GO dataset, we found $\beta = 0.53$, $\sigma = 0.19$, and $\lambda= .75$. Note that we use the ``pure'' EMG distribution here, and not the EMG model with linear conditional mean \autoref{eq:emg_mean}, since copulas do not support a marginal conditional on the other.
    % \item The levels and number of repetitions may also be given to generate repeated measures.
\end{enumerate}

\paragraph{Pro's and con's.} Copulas offer a clean way to specify the dependency between \ide and MT independently of their marginals. This means one can mix and match marginals for \ide (\eg to fit a specific task, for example a realistic \ide distribution may be bimodal or trimodal, as potential targets in an interface are usually clustered in two or three zones such as sidebars, topbars, contextual menus) and MT (\eg using another asymmetric distribution) while keeping the same t-copula to specify their dependence. Thus, copulas provide a flexible framework for generating dependent pointing data.
A potential mismatch between copulas and pointing models, is that they have been used mostly to capture tail dependencies \ie the association between the tails of both marginals, but in pointing tasks, as shown by the EMG model, the lower tail of MT is associated to the entire range of \ide values, and not only its tail (see the ``minimum'' Fitts' law in \autoref{subs:consistency}). Thus, their generative performance needs evaluating.

\subsection{Model 2: Conditional distribution using EMG.}
\paragraph{The model.} The second method directly leverages the chain rule decomosition from \autoref{eq:chain_rule} and the EMG model with quadratic variance. For each point, a value of $\ide$ is selected by sampling from the marginal $F_{\theta}(\ide)$; then a value of MT is selected by sampling from the conditional distribution
\begin{align}
  p_{\theta}(\mt|\ide) = \text{EMG}(\mt; \ide, \theta).
\end{align}
...
\begin{align}
  \lambda_1 = \text{max}(0, \frac{\mmt - a - b\,\ide - \lambda_0}{\ide}). \label{eq:lambda_1}.
\end{align}
We call this model EMG with control.
...
\begin{enumerate}
  ...
  \item ISO-throughput, also known as the ``mean-of-means'' throughput~\cite{soukoreff2004}, which compresses the entire dataset into a single value in bit/second supposed to capture overall pointing performance.
\end{enumerate}

We found that models 2 and 3 could sometimes produce negative MT for very low levels of \ide. Indeed, modeling movement times in the small ID regime is often problematic, since aimed movement for very large targets is more ballistic than corrected~\cite{elliott2017,gori2020}. Using the MacKenzie formulation for ID~\cite{gori2018chi} somewhat relieves that problem. Some researchers prefer to model movement times below ID = 2 as constant duration~\cite{crossman1957}, or only dependent on movement amplitude~\cite{gan1988}.
To solve this problem, we applied a simple post-processing step, where we  discard data points where MT is below 200\,ms, considering that such short movements are not plausible, and without any compensation. The outputs of the models without this post-processing can be found in the supplementary materials, with minimal changes induced by the post-processing.

The generated datasets are shown in \autoref{fig:consistency}. Overall, differences between the models are modest and pass the ``eye test'' when compared to the ground truth data; The association measures also show little to no differences. Regarding the other features:
\begin{itemize}
  \item All models slightly underestimate ISO throughput, with Model 1 deviating the most (by up to 0.27 bit/s).
...
\begin{figure}[htbp]
  \centering
  \makebox[\textwidth]{
    \includegraphics[width=1.2\columnwidth]{img/method_consistency_post_process.pdf}
  }
...
Because our goal is to provide a model that is parametrized by strategy, we next asked whether the generated datasets are consistent with the bivariate Gaussian model when grouped by strategy levels. For each method, we generated data under five strategy levels and refit the bivariate Gaussian model as in \autoref{fig:meancov_strat}.

The results are shown in \autoref{fig:comp_gen_strat}. Model 1 matches well with the bivariate Gaussian model on the \ide dimension, but fails on \mmt, where it does not predict the right correlation with \ide levels, and the correlation parameter, which is much too large.
Models 2 and 3 are much closer overall, with Model 3 providing the best alignment with the Gaussian parametrization. This is not so surprising since it is the only model directly leveraging the Gaussian bivariate model.

\begin{figure}[htbp]
  \centering
  \makebox[\textwidth]{
  \includegraphics[width=1.2\textwidth]{img/tcop_strat_post_process.pdf}}
  \makebox[\textwidth]{
  \includegraphics[width=1.2\textwidth]{img/emg_strat_post_process.pdf }}
  \makebox[\textwidth]{
  \includegraphics[width=1.2\textwidth]{img/emg_control_strat_post_process.pdf }}
...
\begin{itemize}
  ...
  \item The bivariate Gaussian model, described in \autoref{eq:gaussian_ide_mt} together with its parameters relationships to ID and strategy described in \autoref{fig:biv_strategy_fit_yamanaka}, allows generating \ide and \mmt values from nominal values. It provides a relatively simple description, while still capturing effects of strategy and task geometry and part of the variability of human movements. It may thus represent a sweet spot for some researchers.
  \item The three generative models described in \autoref{sec:gen}, with their strengths and weaknesses previously discussed.
\end{itemize}
We believe researchers can often use our reported parameter values if the goal is to predict movement times \eg to design an intelligent system where movement times play a role. As we have used datasets acquired over a variety of conditions (pointing with a mouse, with the limb, or with gestures), we expect our results to be quite robust. However, if the context differs substantially, or to obtain more accurate predictions, these parameters can be re-estimated from new data using standard statistical methods.

\subsection{On the local and global dependency}
In this work, we examined both local and global dependencies between \ide and MT, corresponding to two levels of the speed-accuracy tradeoff (SAT): a local SAT operating within conditions and a global SAT across conditions. Our finding of minimal to no local dependency suggests two possible explanations. One is that movement time variability due to internal processes such as internal motor noise~\cite{vanbeers2004,faisal2008} dwarfs the variability induced by changes in \ide within a single condition. Another is that movements executed under a fixed condition are functionally equivalent. For example, it could be hypothesized that, within a given D-W pair in a classical Fitts' law experiment, people adopt a single motor plan. This hypothesis is supported by the results of the Fitts-with-strategy study, which shows that the tradeoff becomes substantially stronger within a single D-W pair when participants use different strategies.
Thus, the absence of a local speed-accuracy tradeoff could be used as an indicator of the ``stability'' of movements: if a user displays a local speed-accuracy tradeoff, then this could indicate a speed-accuracy ``rebalancing'', where the user shifts strategies between trials. It could reflect whether the user is still learning (\eg when using a novel device, or during rehabilitation~\cite{sungeelee2024}) or signal fatigue or overload.

To our knowledge, this local-global distinction has not been recognized in pointing models before. SAT effects operating at multiple scales have been examined in other domains, most notably reaction time research, where they are referred to as micro and macro SATs~\cite{dennis1996,domingue2022}.
It is notably found that often increases in response time do not lead to increases in accuracy, but that time pressure do consistently reduce accuracy~\cite{domingue2022}. This seems in line with our findings, where an explicit strategy, partly operationalized as a time pressure (be as fast as possible, take your time) leads to a stronger local tradeoff (pure strategy and Fitts-with-strategy protocols) than the one found in the classic Fitts protocol.
Interestingly, but perhaps only coïncidentally, the asymmetric, long-tailed distributions we found to be critical for modeling movement times are also a hallmark of reaction times~\cite{luce1991}.

\subsection{Applicability of copulas}
We introduced copulas as a way to model the joint distribution of \ide and MT, but also as a way to generate new data. To our knowledge, this is the first application of copulas in HCI. The main advantage of copulas is that they separate the dependency structure from the marginals: one can represent the dependency with a compact parametric form (often just one or two parameters) without having to impose restrictive assumptions about the marginal distributions. This is useful when the task changes; we provided an example for bi/tri-modal distributions for \ide in \autoref{sub:model1}.

Admittedly, the case we studied is not the most compelling for copulas; the dependency is already quite well captured by the EMG model, and our goal was not to position copulas as a central contribution of this paper, but rather to use it as a tool to study dependencies with the least amount of assumptions. Nonetheless, we see strong potential for their use in HCI. Interaction, at its core, is about dependencies between variables\footnote{Two variables that are independent can not describe a useful interaction scenario, since they can be understood one without the other.}, and in many situations these dependencies may be nonlinear or dominated by tail behavior (\eg breakdowns). Copulas are designed for precisely such cases. We namely see promising opportunities % in classical movement paradigms such as drawing (\eg the 2/3 power law~\cite{lacquaniti1983}) or steering~\cite{accot1997}, and more generally
...
These models also open a path toward modeling how pointing distributions vary with the perceived cost of errors, as previous work has demonstrated that users modify their pointing strategies in response to perceived error costs~\cite{bertucco2015}.
...
%       \mt = a + b(\text{ID} - I_U).
%     \end{align}
%     \item As discussed \autoref{app:protocols}, movement endpoint variability can be manipulated either by W or by instructions given to the participant. Zhai~\etal{} determined that replacing $W_e$ in \autoref{eq:ide} by
%     \begin{align}
...
\appendix
\makeatletter
\show\subsectionautorefname
\makeatother
\renewcommand{\subsectionautorefname}{Appendix}

\section{Protocols for manipulating the speed accuracy trade-off \label{app:protocols}}
...
\subsection{Fitts' protocol\label{app:protocols::fitts_protocol}}
...
\subsection{Schmidt's protocol\label{app:protocols::schmidt_protocol}}
...
\subsection{Pure strategy protocol\label{app:protocols::guiard_protocol}}
...
\subsection{Fitts-with-strategy protocol\label{app:protocols::full_protocol}}
...
\begin{itemize}
  ...
  \item The law of total expectation
    ...
    \end{align}
  \item The law of total variance\footnote{This formula splits the total variance Y into the variance of group (X) means and the average residual within group (X) variance. It is conceptually similar to a one-way ANOVA, where the total sum of squares is partitioned into within and between group sum of squares.}
    \begin{align}
...
\end{align}
When compared with \autoref{eq:proof_3}, one thus sees that the only difference between $r(X,Y)$ and $r(\overline{X},Y)$ is that $\mathbb{E}[\text{Var}(X|Y)]$ is not present in the denominator. This shows $r(\overline{X},Y)$ is always at least as large as $r(X,Y)$, and explains why considering block averages will lead to ``better'' results.
...
\end{align}
The covariance parts (\ie the numerators in \autoref{eq:r_1} and \autoref{eq:r_2}) simplify as
\begin{align}
...
\begin{align}
  r(X,Y) = \frac{b\sigma_Y^2}{\sigma_Y\sqrt{\mathbb{E}[g(Y)] + b^2\sigma_Y^2}} = \frac{1}{\sqrt{1 + \frac{\mathbb{E}[g(Y)]}{b^2\sigma_Y^2}}}.
\end{align}
The reason that $g(Y)$ does not play a role in the $r^2$ value is that the variance of $X$ for a given $Y=y$ is nullified when considering $\overline{X}$.
...
We considered copulas from the most popular families: elliptical (\eg the Gaussian and $t$ copulas), Archimedean (\eg Clayton, Gumbel), and extreme value (\eg HR, Galambos, $t$-EV), as well as their rotated variants\footnote{Copulas can exhibit dependencies in the lower tails (low values) or upper tails (high values) of distributions. Given that pointing data in our dataset displays high variance at higher ID levels, copulas with upper tail dependence, such as the Gumbel copula, tend to perform poorly. By rotating the copula, we switch the dependence from the upper tail to the lower tail, providing additional copula candidates for consideration.}, and the independent copula.
We utilized the R \texttt{copula} package~\cite{yan2007} to fit these candidate copulas; copulas are estimated based on maximum likelihood estimation, and the marginals are estimated with \textit{empirical} maximum likelihood estimation, as illustrated in the code that comes with this paper.
Sometimes, fits were unsuccessful; this can happen when the copulas are fit to data whose dependence they can not represent, such as negative rank-correlated data with a copula that can only express positive rank-correlated data.

\subsection{Comparison procedure\label{app:copula:compare}}
To compare copulas, we use the model evidence ratio $\mathcal{R}$, which builds on AIC. A full exposition of model comparison based on AIC and $\mathcal{R}$ can be found in ~\cite{anderson2004} but is summarized here for convenience. In the maximum likelihood estimation approach, the combination of model and parameter values that reaches the maximum log-likelihood $\mathcal{L}$ is preferred. This leads to overfitting, as a nested model with fewer parameters can only do worse than the full model. One solution to that is AIC, a score that penalizes $\mathcal{L}$ with the number of parameters $k$. A model's AIC reads
\begin{align}
  AIC = 2(k - \mathcal{L});
\end{align}
a lower AIC indicates better fitting power. Models can then be compared based on their AIC difference, generally, an AIC difference of 10 is considered very strong support~\cite{anderson2004} \ie the competing model is implausible. An interesting transformation of AIC differences is the relative likelihood ratio or model evidence ratio $\mathcal{R}$
\begin{align}
  \mathcal{R} = \exp \left(- \frac{\text{AIC}_1 - \text{AIC}_2}{2} \right)
\end{align}
which can directly be interpreted as odds (see also~\cite{wagenmakers2004}).

One value of $\mathcal{R}$ is computed per fit \eg for one participant in one condition. Can we combine various $\mathcal{R}$ values to give a global comparison across the entire dataset?
Yes, and the existing literatures lists three strategies:
\begin{itemize}
  ...
  \item Compute the loglikelihoods $\mathcal{L}$ for each dataset, and sum them to get a single $\mathcal{L}$. This is equivalent to pooling the data $\mathcal{L}$, but with the difference that we allow each sub-dataset to be fitted by a model (copula) with different parameters. It is thus best suited if one expects some heterogeneity between datasets.
  \item Compute the loglikelihoods $\mathcal{L}$ for each dataset, and average them to get a single $\mathcal{L}$. This is equivalent to the previous method in terms of comparisons, except with the presence of NaNs when a fit failed (see Appendix \ref{app:copula:fit}).
\end{itemize}
Since we expected differences between experimental conditions, we did not pool the data. Instead, we considered the sum of the likelihoods, which penalizes models that fail the fits, compared with the averages, which by default, exclude the NaNs before averaging.
...
\section{Extra figure: Generating pointing data, \ide included}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=.8\textwidth]{img/sampled_ide.pdf}
  \caption{The pointing data from \autoref{fig:consistency} for Model 3 (EMG with control), but with \ide values sampled rather than taken from the ground truth data. Notice that the performance is very similar, which warrants our use of the ground truth data.}
