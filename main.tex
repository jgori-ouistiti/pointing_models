...
In a second part, we shift to a predictive perspective: using what we have learned about these dependencies, we build and compare three generative models of pointing.
...
The key finding from \citeauthor{zhai2004nominal}~\cite{zhai2004nominal} for our purposes is that the post-hoc correction \ide depends on user strategies. Moreover, they showed that D and W have effects on movement time beyond through their ratio D/W. These results imply that \ide should be treated as a quantity shaped jointly by ID, D, W, and participant strategy. This observation motivates our modeling of its distribution, see \autoref{sec:outline}.
...
\end{align}
where erfc is the complimentary error function, $\mathbf{x} = \left[1, \ide \right]^T$, $\lambda = \left[ \lambda_0, \lambda_1 \right]$ and $\beta = \left[ a, b\right]$. 
...
We fitted the EMG model for each participant and for each replication. In line with existing works on this dataset~\cite{gori2018these,zhao2022}, the EMG fit was largely superior to the Gaussian fit obtained via linear regression. We namely found AIC\footnote{Akaike Information Criterion, a standard metric for model comparison~\cite{anderson2004}, see also \autoref{app:copula:compare}.} differences routinely above 300; typically a difference of 10 is already considered strongly discriminative~\cite{anderson2004}. A pair-plot of the fitted parameters can be found in the supplementary materials.
...
We compared candidate copula models using the model evidence ratio $\mathcal{R}$ as explained in \autoref{app:copula:compare}. 
...
The two simple model fits are shown in \autoref{fig:ide_id_jpg}, which illustrates that D significantly shifts the \ide - ID relationship upwards. This is consistent with classical results from motor control where longer distances lead to larger variability in movement trajectories~\cite{gori2020}. 
...
For comparison with the two previous studies, we only used regular (mouse-based) pointing data, and aggregated data across distractor density levels, since the sensitivity of MT in regular pointing to distractor density if low~\cite[Table 2]{blanch2011}.
...
\begin{figure}[htbp]
   ...
   \begin{subfigure}[t]{0.43\textwidth}
	...
	\caption{Data of the YKORM dataset and adjusted Fitts' law obtained by computing linear regression on MT with \ide and strategy as predictor. The fitted model without noise is MT = -0.24 + 0.266$\cdot{}\ide$ -0.244$\cdot{}s$ + 0.073$\cdot{}s\cdot{}\ide$.}
...
Across the three studies, copulas consistently revealed a weak local dependence between \ide and MT. This suggests that faster movements can also be more precise, and that this may actually be relatively common. The pure strategy protocol introduced a somewhat stronger local dependence; this suggests that when accuracy is enforced by W, movement are largely ``equivalent'', whereas when accuracy is enforced by strategy, the participants seem to exerce more control, we believe \eg by re-adjusting the strategy based on the previous trial. 
...
\end{align}
Pairs of (MT,\ide) values can then be obtained by sampling directly from the joint. If the goal is to replicate blocked data \ide have several values of MT associated with a single \ide value, then one can first select \ide values by sampling from the $p(\ide|s)$ model, and them sampling MT from the \textit{conditional} copula, which can be computed directly from the relevant copula~\cite{nelsen2006}.
...
A potential flaw of copulas is that they have been used mostly to capture tail dependencies, but in pointing tasks, the lower tail of MT is linked to the entire range of \ide values (see the ``minimum'' Fitts' law in \autoref{subs:consistency}), so their generative performance needs evaluation. 
...
\begin{itemize}
	...
	\item Compute the loglikelihoods $\mathcal{L}$ for each dataset, and average them to get a single $\mathcal{L}$. This is equivalent to the previous method in terms of comparisons, except with the presence of NaNs when a fit failed (see \autoref{app:copula:fit}).
\end{itemize}
